...
namespace OptiRoute.Forms
{
    public class DriverDashboardForm : Form
    {
        ...
        private void RefreshDashboard()
        {
            ...
            // ':F1' lagane se decimal ke baad sirf 1 digit show hoga (e.g., 80.2%)
            vCard.Controls.Add(L(fuel.ToString("F1") + "%", new Font("Segoe UI", 9f, FontStyle.Bold), fuelColor, new Point(trackW + 26, 115)));
            // ── Rating card — right half ──────────────────────────
            var rCard = Card(30 + halfW + gap, row2Y, halfW, 160);
            pnlDashboard.Controls.Add(rCard);
bool hasDeliveries = stats.TotalRatings > 0;
...
double avg = hasDeliveries
    ? _driverRepo.GetLiveAverageRating(_driver.UserID)
    : 5.0;
int fullS = hasDeliveries ? (int)Math.Round(avg) : 5;
        ...
        private void LoadAssignmentCards()
        {
            ...
            foreach (Order o in orders)
            {
                ...
                {
                    Text = "📥  Pick Order",
                    Location = new Point(cardW - 360, 115),
                ...
                btnPick.MouseEnter += (s, e) => {
                    cardHoverAll(true);
                    if (curStatus == "Assigned") btnPick.ForeColor = Color.White;
                };

                // Mouse baahar jaye toh card hover band ho aur text wapas purana (RoyalBlue) ho jaye
                btnPick.MouseLeave += (s, e) => {
                    var pos = card.PointToClient(Control.MousePosition);
                    if (!card.ClientRectangle.Contains(pos)) cardHoverAll(false);
                    if (curStatus == "Assigned") btnPick.ForeColor = RoyalBlue;
                };
                btnPick.Click += (s, e) =>
                {
                ...
                {
                    Text = "✅  Deliver",
                    Location = new Point(cardW - 190, 115),
                    Size = new Size(160, 40),
                    FlatStyle = FlatStyle.Flat,
                    Font = new Font("Segoe UI", 10f, FontStyle.Bold),
...
function moveTruck(lat,lng,step,total){{
  ...
  document.getElementById('fill').style.width = pct+'%';
        ...
        private void BuildProfilePanel()
        {
            ...
            int vInputW = (vCardW - 60) / 2;

            vCard.Controls.Add(L("Number Plate", new Font("Segoe UI", 9.5f, FontStyle.Bold), TextGray, new Point(20, 46)));
            var txtPlate = new TextBox { Text = _driver.PlateNumber, Location = new Point(20, 66), Size = new Size(vInputW, 30), Font = new Font("Segoe UI", 11f), BorderStyle = BorderStyle.FixedSingle, BackColor = CardBg };
            vCard.Controls.Add(txtPlate);

            vCard.Controls.Add(L("Vehicle Type", new Font("Segoe UI", 9.5f, FontStyle.Bold), TextGray, new Point(40 + vInputW, 46)));
            var cmbType = new ComboBox { Location = new Point(40 + vInputW, 66), Size = new Size(vInputW, 30), DropDownStyle = ComboBoxStyle.DropDownList, Font = new Font("Segoe UI", 11f), BackColor = CardBg, ForeColor = TextDark };
            ...
            int pwInputW = (pwCardW - 64) / 3;

            pwCard.Controls.Add(L("Current Password", new Font("Segoe UI", 9f, FontStyle.Bold), TextGray, new Point(22, 46)));
            var txtCurPw = new TextBox { Location = new Point(22, 66), Size = new Size(pwInputW, 30), Font = new Font("Segoe UI", 11f), BorderStyle = BorderStyle.FixedSingle, BackColor = CardBg, UseSystemPasswordChar = true };
            pwCard.Controls.Add(txtCurPw);

            pwCard.Controls.Add(L("New Password", new Font("Segoe UI", 9f, FontStyle.Bold), TextGray, new Point(22 + pwInputW + 10, 46)));
            var txtNewPw = new TextBox { Location = new Point(22 + pwInputW + 10, 66), Size = new Size(pwInputW, 30), Font = new Font("Segoe UI", 11f), BorderStyle = BorderStyle.FixedSingle, BackColor = CardBg, UseSystemPasswordChar = true };
            pwCard.Controls.Add(txtNewPw);

            pwCard.Controls.Add(L("Confirm Password", new Font("Segoe UI", 9f, FontStyle.Bold), TextGray, new Point(22 + (pwInputW + 10) * 2, 46)));
            var txtConfPw = new TextBox { Location = new Point(22 + (pwInputW + 10) * 2, 66), Size = new Size(pwInputW, 30), Font = new Font("Segoe UI", 11f), BorderStyle = BorderStyle.FixedSingle, BackColor = CardBg, UseSystemPasswordChar = true };
            ...
            var btnUpdate = new Button { Text = "Update", Location = new Point(pwCardW - 130, 105), Size = new Size(108, 35), FlatStyle = FlatStyle.Flat, Font = new Font("Segoe UI", 9.5f, FontStyle.Bold), BackColor = RoyalBlue, ForeColor = Color.White, Cursor = Cursors.Hand }; btnUpdate.FlatAppearance.BorderSize = 0;
            btnUpdate.Click += (s, e) =>
            {
                lblPwStatus.Text = "";
