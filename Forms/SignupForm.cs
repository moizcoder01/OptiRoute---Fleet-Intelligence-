...
namespace OptiRoute.Forms
{
    public class SignUpForm : Form
    {
        ...
        private void BtnSignUp_Click(object? sender, EventArgs e)
        {
            ...
            if (role == "Customer")
            {
                newID = _userRepo.RegisterCustomer(
                    firstName, lastName, username,
                    hashedPassword, email, phone);
            }
            else // Driver
            {
                // Collect driver-specific details via input boxes
                string license = Microsoft.VisualBasic.Interaction.InputBox(
                    "Enter your License Number:", "Driver Details", "");

                if (string.IsNullOrWhiteSpace(license))
                {
                    ShowWarning("License number is required for drivers.", "Incomplete");
                    return;
                }

                string plate = Microsoft.VisualBasic.Interaction.InputBox(
                    "Enter your Vehicle Plate Number:", "Driver Details", "");

                if (string.IsNullOrWhiteSpace(plate))
                {
                    ShowWarning("Vehicle plate number is required for drivers.", "Incomplete");
                    return;
                }

                string vtype = Microsoft.VisualBasic.Interaction.InputBox(
                    "Enter Vehicle Type (Bike / Car / Van / Truck):",
                    "Driver Details", "Bike");

                if (string.IsNullOrWhiteSpace(vtype))
                    vtype = "Bike";

                newID = _userRepo.RegisterDriver(
                    firstName, lastName, username,
                    hashedPassword, email, phone,
                    license, plate, vtype);
            }
